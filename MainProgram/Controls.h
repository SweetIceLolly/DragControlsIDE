...
const UINT MY_DEBUGGER_BREAKPOINT = 0x8888;		//断点命中消息
const UINT MY_DEBUGGER_MEMDATA = 0x9999;		//内存读取消息
const HWND DEBUGGER_HWND = (HWND)0xF05EC;		//【DebuggerHwnd】

//滚动条移动速度常数
const int  HS_LargeChange[1] = { 10 };
const int  HS_SmallChange[1] = { 1 };
const int  VS_LargeChange[1] = { 10 };
const int  VS_SmallChange[1] = { 1 };
...
/*---------------------------------------------------------------------*/
/* 以下是所有控件事件的函数原型 */
void IpAddress_1_Change();								//IpAddress 1 更改事件
/*=====================================================================*/
/* 计算当前的Shift值 */
int GetShiftValue()
{
...
public:
	...
	HWND Create()
	{
		CurrentHwnd = CreateWindowEx(0, "STATIC", "", WS_VISIBLE | WS_CHILD | SS_BLACKFRAME | SS_NOTIFY,
			Left, Top, Width, Height, GetCurrentHwnd(), (HMENU)hMenu, GetCurrentHinstance(), 0);			//创建控件
...
public:
	...
	*/
	...
	{
		DWORD CurrSel = SendMessage(CurrentHwnd, EM_GETSEL, 0, 0);
		DWORD CurrLnSel = SendMessage(CurrentHwnd, EM_LINEINDEX, -1, 0);
...
public:
	...
	HWND Create()
	{
		...
		//创建控件
		CurrentHwnd = CreateWindowEx(ClientEdgeBorder ? WS_EX_CLIENTEDGE : 0, "MyButton", Text, lStyle,
			Left, Top, Width, Height, GetCurrentHwnd(), (HMENU)hMenu, GetCurrentHinstance(), 0);
...
public:
	...
	*/
	...
	{
		return (int)SendMessage(CurrentHwnd, CB_SETCURSEL, (WPARAM)ListIndex, 0);
	}
